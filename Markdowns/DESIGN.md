...
- **Hamburger menu** slides in from the left with a slight staggered fade per row. Navigation only: Browse, My bookings, Hosting for hosts, Create listing / Become a host for every signed-in user, Verification review for admins. Account actions live on the avatar. Signed-out: Log in or sign up, Browse, and the three policy links.
